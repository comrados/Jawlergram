...
public class MediaDownloadMethods {
     ...
     * @param maxSize max size of medial to be saved
     */
    public static String messageDownloadMediaToDB(TelegramApi api, DBStorage dbStorage, TLAbsMessage absMessage,
                                                  int maxSize) {
        String out = null;
        if (absMessage instanceof TLMessage) {
            if (((TLMessage) absMessage).hasMedia()) {
     ...
     * @param path path on the HDD
     */
    public static String messageDownloadVoiceMessagesToHDD(TelegramApi api, TLAbsMessage absMessage,int maxSize,
                                                           String path) {
        String out = null;
        if (absMessage instanceof TLMessage) {
            if (mediaIsVoiceMessage((TLMessage) absMessage)) {
                byte[] bytes = messageGetMediaBytes(api, (TLMessage) absMessage, maxSize);
                String name = getFileName((TLMessage) absMessage);
                if ((name != null) && (bytes != null)){
                    String filePath = FileMethods.setFileNameAndPath(name, path);
    ...
    /**
     * Checks if message is audio message
     *
     * @param message message
     */
    private static boolean mediaIsVoiceMessage(TLMessage message){
        TLAbsMessageMedia absMedia = message.getMedia();
        if (absMedia instanceof TLMessageMediaDocument){
            TLAbsDocument absDoc = ((TLMessageMediaDocument) absMedia).getDocument();
            if (absDoc instanceof TLDocument){
                TLVector<TLAbsDocumentAttribute> attributes = ((TLDocument) absDoc).getAttributes();
                for (TLAbsDocumentAttribute attribute: attributes){
                    if (attribute instanceof TLDocumentAttributeAudio){
                        return ((TLDocumentAttributeAudio) attribute).isVoice();
                    }
                }
            }
        }
